...
export default function PlaceOrderScreen() {
  ...
  cart.voucherSales =
    Object.keys(cart.paymentInfo.voucher).length !== 0
      ? (cart.itemsPrice * cart.paymentInfo.voucher.discount) / 100
      : 0;
  ...
  const placeOrderHandler = async () => {
    try {
      ...
      if (cart.paymentInfo.voucher._id)
        await Axios.patch(
          `/api/vouchers/${cart.paymentInfo.voucher._id}`,
      ...
      alert(
        "Thanh toán thành công, bạn sẽ được chuyển đến trang chi tiết của đơn hàng này"
      );
            ...
            <Card className="p-3 mb-3 border-dark ord-card--darker">
              <Card.Body>
                <div className="d-flex justify-content-between align-items-center mb-2">
                  <Card.Title className="ord-card-title">
              ...
              <Card.Body>
                ...
                <Card.Text as="div">
                  ...
                  <Row className="my-2">
                    ...
                    <Col xs={10}>
                      {cart.paymentInfo.voucher.code || ""} -{" "}
                      {cart.paymentInfo.voucher.name || ""}
                    </Col>
                  </Row>
                </Card.Text>
              </Card.Body>
